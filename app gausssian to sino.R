...
server <- function(input,output,session){
  ...
  uelw <- reactive({
    ...
    # Gaussian changed to Sine Wave, one pathway
    if (input$fn == "1" && input$mpath_c == FALSE) {
      t_plot <- t_plot + map(1:(nrow(df_up_e()) - 1), ~ {
        j <- .x
        stat_function(fun = function(x) {
          amplitude <- df_up_e()$A[j]
          frequency <- df_up_e()$B[j]
          phaseShift <- df_up_e()$C[j]
          verticalShift <- df_up_e()$D[j]
          
          # Sine Wave Formula: y = A * sin(B * (x - C)) + D
          amplitude * sin(frequency * (x - phaseShift)) + verticalShift
        }, 
